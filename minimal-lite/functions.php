...
function minimal_lite_scripts() {
    ...
    wp_enqueue_style( 'minimal-lite-style', get_stylesheet_uri(), array() , '1.1.8' );
	...
	wp_enqueue_script('script', get_template_directory_uri() . '/assets/thememattic/js/script.js', array( 'jquery', 'wp-mediaelement' ), '1.1.8', true);
...
function minimal_lite_post_nav_background() {
    ...
    if ( $next && has_post_thumbnail( $next->ID ) ) {
		...
		if(isset($nextthumb[0] )){
			$next_url = $nextthumb[0] ;
		}
        $css .= '
			.post-navigation .nav-next { background-image: url(' . esc_url( $next_url ) . '); border-top: 0; }
